...
function _loadLogo() {
  ...
  // First try: grab the already-loaded header logo (avoids CORS issues entirely)
  const headerLogo = document.querySelector(".header-logo-img");
  if (headerLogo && headerLogo.naturalWidth > 0 && headerLogo.complete) {
    _logoImg = headerLogo;
    return Promise.resolve(headerLogo);
  }
  // Fallback: load fresh (same-origin, no crossOrigin to avoid CORS failures)
...
export async function buildVerdictImage(barcode, status, productName, brand, reasons, statusMeta, explain) {
  ...
  // Status badge for non-GREEN (RESTRICTED / AMBIGUOUS / NOT SUITABLE)
  if (status !== "GREEN") {
    const badgeLabels = {
      RED: "NOT SUITABLE", ORANGE: "AMBIGUOUS", YELLOW: "RESTRICTED", UNKNOWN: "UNKNOWN",
    };
    const badgeText = badgeLabels[status] || status;
    ctx.font = "bold 11px system-ui, sans-serif";
    const badgeW = ctx.measureText(badgeText).width + 16;
    const badgeX = 30 + ctx.measureText(label).width + 12;
    // Re-measure label width with the correct font
    ctx.font = "bold 30px system-ui, sans-serif";
    const labelW = ctx.measureText(label).width;
    const bx = 30 + labelW + 12, by = 128, bh = 22;
    ctx.font = "bold 11px system-ui, sans-serif";
    ...
    if (ctx.roundRect) { ctx.roundRect(bx, by, bw, bh, 11); }
      ...
      ctx.moveTo(bx + 11, by);
      ctx.arcTo(bx + bw, by, bx + bw, by + bh, 11);
      ctx.arcTo(bx + bw, by + bh, bx, by + bh, 11);
      ctx.arcTo(bx, by + bh, bx, by, 11);
      ctx.arcTo(bx, by, bx + bw, by, 11);
      ctx.closePath();
    }
    ctx.fill();
    ctx.fillStyle = "#fff";
